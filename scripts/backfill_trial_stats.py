...
async def backfill(*, data_dir: Path, test_results_dir: Path) -> tuple[int, int]:
    """Insert snapshots into ``data_dir/state.db``, skipping existing run IDs."""
    inserted = 0
    skipped = 0
    engine = create_async_engine(f"sqlite+aiosqlite:///{data_dir / 'state.db'}", future=True)
    ...
    try:
        async with session_factory() as session:
            ...
            strategy_by_slug = {
                slug: (strategy_id, family) for slug, strategy_id, family in strategies
            }
